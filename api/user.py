...
async def create_flight_histiory(data: FlightHistoryInfo):
    return await User.create("flight_history", data)


@router.get("/get_all_flight_history", response_model=list[GetFlightHistory], status_code=status.HTTP_200_OK)
async def get_all_flight_history():
    return await User.get_all("flight_history")
